...
GSLChannel::TransmitStart(
    ...
    Time txTime)
{
    NS_LOG_FUNCTION(this << p << src << dst_address);

    // 转换为 Mac48Address
    Mac48Address dstMac = Mac48Address::ConvertFrom(dst_address);

    // 广播处理
    if (dstMac == Mac48Address::GetBroadcast())
    {
        NS_LOG_DEBUG("GSLChannel: Broadcasting packet from node " << src->GetNode()->GetId());

        for (auto const &entry : m_link)
        {
            Ptr<GSLNetDevice> dst = entry.second;
            if (dst == src) continue;
            ...
            {
                NS_LOG_WARN(
                    "Skipping broadcast to node "
                    << dst->GetNode()->GetId()
                    << " (different system, distributed mode not supported)."
                );
                continue;
            }
    ...
    // 单播处理
    if (dstMac != Mac48Address()) // 非空地址
    {
        auto it = m_link.find(dstMac);
        if (it != m_link.end())
        {
    ...
    // 找不到目标 MAC
    NS_LOG_WARN("TransmitStart: MAC address " << dst_address
                << " could not be mapped to a network device. Packet dropped.");
    return false;
}
