...
PointToPointLaserNetDevice::TransmitComplete (void)
{
  ...
  // Notify upper-layer UDP sockets on this node that queue space became
  // available. This helps applications that rely on Socket::SetSendCallback
  // to be resumed when the device frees transmit queue space.
  Ptr<Node> node = this->GetNode ();
  if (node != 0)
    {
      Ptr<ns3::UdpL4Protocol> udp = node->GetObject<ns3::UdpL4Protocol> ();
      if (udp != 0)
        {
          NS_LOG_INFO ("LaserNetDevice: NotifySocketsCanSend on node " << node->GetId ());
          udp->NotifySocketsCanSend (1);
        }
      else
        {
          NS_LOG_INFO ("LaserNetDevice: node " << node->GetId () << " has no UdpL4Protocol object");
        }
    }
...
{
  NS_LOG_UNCOND("[LASER-RX] node=" << GetNode()->GetId()
                   << " uid=" << packet->GetUid()
                   << " size=" << packet->GetSize());
...
bool
PointToPointLaserNetDevice::Send(Ptr<Packet> packet,
                                 const Address &dest,
                                 uint16_t protocolNumber)
{
  // -------------------------
  // Fundamental pointer checks
  // -------------------------
  if (!packet)
    {
      NS_LOG_UNCOND("[LASER-TX] ERROR: Send() got NULL packet — DROP");
      return false;
    }

  if (m_queue == nullptr)
    {
      NS_LOG_UNCOND("[LASER-TX] ERROR: m_queue is NULL — DROP packet UID=" << packet->GetUid());
      return false;
    }

  if (m_channel == nullptr)
    {
      NS_LOG_UNCOND("[LASER-TX] ERROR: m_channel is NULL — DROP packet UID=" << packet->GetUid());
      return false;
    }

  Ptr<Node> node = GetNode();
  uint32_t nodeId = node ? node->GetId() : 999999;

  if (!node)
    {
      NS_LOG_UNCOND("[LASER-TX] ERROR: GetNode()==NULL — DROP");
      return false;
    }

    

  NS_LOG_UNCOND("[LASER-TX] node=" << nodeId
                   << " ifindex=" << GetIfIndex()
                   << " uid=" << packet->GetUid()
                   << " size=" << packet->GetSize());
  ...
  if (!IsLinkUp())
    {
      NS_LOG_UNCOND("[LASER-TX] Link is DOWN — DROP UID=" << packet->GetUid());
      m_macTxDropTrace(packet);
      return false;
    }

  // ----------------------
  // Add PPP header safely
  // ----------------------
  ...
  if (!m_queue->Enqueue(packet))
    {
      NS_LOG_UNCOND("[LASER-QUEUE] Queue overflow — DROP UID=" << packet->GetUid());
      m_macTxDropTrace(packet);
      return false;
    }

  NS_LOG_UNCOND("[LASER-QUEUE] ENQUEUE node=" << GetNode()->GetId()
                   << " uid=" << packet->GetUid()
                   << " qlen=" << m_queue->GetNPackets());

  // ----------------------
  // If not currently sending, dequeue & transmit
  // ----------------------
  if (m_txMachineState == READY)
    {
      ...
      if (!p)
        {
          NS_LOG_UNCOND("[LASER-QUEUE] ERROR: Dequeue returned NULL!");
          return false;
        }

      NS_LOG_UNCOND("[LASER-QUEUE] DEQUEUE node=" << GetNode()->GetId()
                       << " uid=" << p->GetUid()
                       << " qlen=" << m_queue->GetNPackets());
      ...
      // Start TX
      // ----------------------
      bool ok = TransmitStart(p);

      NS_LOG_UNCOND("[LASER-TX] TransmitStart returned " << (ok ? "true" : "false"));
                                      ...
                                      uint16_t protocolNumber)
{
    ...
    // 如果 source 地址与设备地址不一致，可以打印警告，但仍使用本地地址发送
    if (Mac48Address::ConvertFrom(source) != m_address)
    {
        NS_LOG_WARN("SendFrom(): source address ignored, using device MAC");
    }

    // 设置目标节点
    SetDestinationNode(GetChannel()->GetDevice(0) == this ? GetChannel()->GetDevice(1)->GetNode()
                                                          : GetChannel()->GetDevice(0)->GetNode());
